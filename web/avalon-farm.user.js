 ...
 */
function foodToKeep(contents, keepQty = KEEP_QUANTITY) {
  const held = new Map();
 ...
 */
function depotSlot(depot, maxDepth = MAX_NEST_DEPTH) {
 ...
 */
function depotFree(depot, maxDepth = MAX_NEST_DEPTH) {
 ...
 */
function nearestAttacker(bot, snap, me, attackerPx = ATTACKER_PX) {
  let best = null;
  for (const m of snap.monsters) {
    if (m.hp <= 0) continue;
